...
local function calculatePartyChecksum()
    ...
    -- Ensure memory reader is initialized
    if not memory_reader.getStatus().initialized then
        return 0
    end
    ...
    if party_count > 0 then
        ...
        -- Read current HP as two bytes
        local hp_low = memory.readbyte(first_pokemon_addr + memory_reader.addresses.current_hp_offset)
        local hp_high = memory.readbyte(first_pokemon_addr + memory_reader.addresses.current_hp_offset + 1)
        checksum = checksum + hp_low + (hp_high * 256)
    end
